...
#    Tcl global array named ::ahttpd::auth$filename, and upvar aliases this 
#    to "info".  "info" contains the info provided by the .htaccess file 
#    ( info(htaccessp,..) )
#
#    The AuthUserFile and the AuthGroupFile are parsed and stored
#    in authentication array ::ahttpd::auth$filename.
#
...
#
#    Configuration creates an authentication array authdefault, for 
#    bootstrapping.
#
...
snit::type ::ahttpd::auth {
    pragma -hasinstances no

    #-------------------------------------------------------------------
    # Type Variables

    # config: Module Data Array.
    #
    # Normally this would be "info", but this module already uses that
    # name.
    #
    # digestOnly  - Whether to use only Digest authentication or not.
    # userFile    - User authentication file
    # groupFile   - Group authentication file
    # defaultFile - Default authentication file (set by init)
    ...
    typemethod init {} {
        upvar #0 ::ahttpd::authdefault authdefault

        # Authentication bootstrap.
        #
        # The /htaccess URL allows admins to create passwords and groups,
        # and .htaccess files per directory, however it is necessary to 
        # have a password to bootstrap this process.
        #
        ...
        # Generate a default for webmaster and write it to a file.
        set config(defaultFile) [fileutil::tempfile tclhttpd.default]    
        set webmaster_password [passgen generate]
        set salt [passgen salt]
        set authdefault(user,webmaster) [crypt $webmaster_password $salt]
        set authdefault(group,webmaster) webmaster
        set fd [open $config(defaultFile) w 0660]
        puts $fd $webmaster_password
        close $fd
        unset webmaster_password
    }

    # check sock directory pathlist
    #
    ...
    # It returns a "cookie" that is checked by [auth verify].
    #
    # NOTE: this looks for the lowest (i.e., deepest) access file
    # and only returns information about one. Consider changing
    # [auth check]/[auth verify] to check all files.

    typemethod check {sock directory pathlist} {
        set cookie {}
        ...
        # Look for the .htaccess files that keep Basic Authentication info
    ...
    # verify sock cookie
    #
    # sock  - A client socket
    # cookie - An [auth check] cookie.
    #
    # Verifies an authentication cookie

    typemethod verify {sock cookie} {
        if {[llength $cookie] == 0} {
            return 1
        }
    ...
    # Auth_VerifyCallback sock realm callback 
    #
    # sock        - Handle on the client connection
    ...
    # to verify the password.  Returns 1 for success and 0 for access 
    # denied.
    ...
    proc AuthVerifyBasic {sock file} {
        ...
        if {[info exists data(mime,authorization)]} {
            ...
            switch -- [string tolower $type] {
                ...
                default {
                    # this is an unknown auth method - it's not ok, 
                    # we can't handle it
                    set ok 0
                }
            }
    ...
    proc AuthGroupCheck {sock file groups user} {
        upvar #0 ::ahttpd::auth$file info
        upvar #0 ::ahttpd::auth$info(htaccessp,groupfile) group
    ...
    # The information is built up in the array:
    #   ::ahttpd::auth$info(htaccessp,userfile)
    # 
    # To create an authentication array, unconnected to a physical file:
    #
    #   array set ::ahttpd::auth$pseudo \
    #       [list mtime 0 user,$u password group,$g group]
    #
    # If the .htaccess file didn't specify a userfile, it defaults to an 
    # authentication array authdefault, which contains configured 
    # default passwords and groups

    proc AuthGetPass {sock file user} {
        upvar #0 ::ahttpd::auth$file info
        upvar #0 ::ahttpd::auth$info(htaccessp,userfile) passwd
    ...
    proc AuthVerifyNet {sock file op} {
        ...
        if {! $ok} {
            ::ahttpd::log add $sock AuthVerifyNet \
                "access denied to $rname in [file tail [file dirname $file]]"
        }
        return $ok
    }
    ...
    proc AuthParseHtaccess {sock file} {
        ...
        if {![info exists info] || ($mtime > $info(htaccessp,mtime))} {
            ...
            set info(htaccessp,userfile)  $config(userFile)
            set info(htaccessp,groupfile) $config(groupFile)
